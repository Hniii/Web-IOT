<?php
   ...
   $composants = $pdo->query( "SELECT composant.*, categorie.libelle as 'categorie_libelle' FROM composant INNER JOIN categorie ON composant.id_categorie=categorie.id WHERE etat like '%$search%' or date like '%$search%' or nom like '%$search%'   or categorie.libelle like '%$search%'or quantite like '%$search%' ")->fetchAll(PDO::FETCH_OBJ); // we can just use query no prblm cause we don't need user data 
